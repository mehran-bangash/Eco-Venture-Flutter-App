...
class TeacherEditStoryScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TeacherEditStoryScreen> createState() => _TeacherEditStoryScreenState();
}

class _TeacherEditStoryScreenState extends ConsumerState<TeacherEditStoryScreen> {
  // --- PRO COLORS ---
  ...
  Future<void> _updateStory() async {
    ...
    final updatedStory = widget.storyData.copyWith(
      ...
      thumbnailUrl: _coverImage?.path ?? _existingCoverUrl, // Prefer new file, else keep old URL
      pages: _pages,
      // 'uploadedAt' and 'id' are preserved from original model via copyWith
    );

    await ref.read(teacherMultimediaViewModelProvider.notifier).updateStory(updatedStory);
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Column(
            children: [
              IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _showPageEditor(existingIndex: index)),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        final String newPath = pageImageFile?.path ?? pageImageUrl ?? "";

                        final newPage = StoryPage(text: textCtrl.text, imageUrl: newPath);
