...
class TeacherAddTreasureHuntScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TeacherAddTreasureHuntScreen> createState() => _TeacherAddTreasureHuntScreenState();
}

class _TeacherAddTreasureHuntScreenState extends ConsumerState<TeacherAddTreasureHuntScreen> {
  ...
  Future<void> _saveHunt() async {
    ...
    await ref.read(teacherTreasureHuntViewModelProvider.notifier).addHunt(newHunt, null);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Add QR Hunt", style: GoogleFonts.poppins(color: _textDark, fontWeight: FontWeight.bold, fontSize: 18.sp)),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text("Create a task. Each clue generates a unique QR code.", style: GoogleFonts.poppins(fontSize: 14.sp, color: Colors.grey[600])),
            SizedBox(height: 3.h),
            ...
            SizedBox(height: 2.h),
            _buildLabel("Points"),
            _buildTextField(_pointsController, "e.g., 100", isNumber: true),
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text("Clue #${index + 1}", style: GoogleFonts.poppins(fontSize: 15.sp, fontWeight: FontWeight.bold, color: _primary)),
                          if (index > 0)
                            InkWell(
                      ...
                      SizedBox(height: 1.h),
                      _buildTextField(_clueControllers[index], "Hint: 'Look under the big tree'", maxLines: 2),
                    ],
                  ),
                ),
              );
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 2.w),
                    Text("Add Next Clue", style: GoogleFonts.poppins(fontSize: 15.sp, fontWeight: FontWeight.bold, color: _primary)),
                  ],
                ),
              ),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: _primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 5),
                child: state.isLoading
                    ? const CircularProgressIndicator(color: Colors.white)
                    : Text("Save & Publish Hunt", style: GoogleFonts.poppins(fontSize: 16.sp, fontWeight: FontWeight.bold, color: Colors.white)),
              ),
            ),
            SizedBox(height: 5.h),
          ],
  ...
  Widget _buildTextField(TextEditingController ctrl, String hint, {bool isNumber = false, int maxLines = 1}) {
    return TextField(
      controller: ctrl, keyboardType: isNumber ? TextInputType.number : TextInputType.text, maxLines: maxLines, style: GoogleFonts.poppins(fontSize: 15.sp),
  ...
  Widget _buildDropdown() {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 4.w), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: _border)),
      child: DropdownButtonHideUnderline(child: DropdownButton<String>(value: _difficulty, isExpanded: true, items: _difficultyLevels.map((c) => DropdownMenuItem(value: c, child: Text(c, style: GoogleFonts.poppins(fontSize: 15.sp)))).toList(), onChanged: (v) => setState(() => _difficulty = v!))),
    );
  }
}